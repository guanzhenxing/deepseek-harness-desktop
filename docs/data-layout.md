...
| `<home>/run/compatibility.json`                                      | 受支持写入者（M4 起 writer） | 任何受支持入口写入前执行固定链：parse marker → 只读格式勘察 → 纯预检 → 原子预约 marker（fsync temp+rename+目录 fsync）；缺失允许，未知 schema/损坏/不支持 epoch/未知格式/不可读格式/需迁移一律拒绝（exit/恢复页 fail-closed），拒绝不触碰任何数据文件 | 预约后启动失败不回滚 epoch；损坏即拒绝不自动重写（[home-compatibility 协议](protocols/home-compatibility.md)、[ADR-0009](adr/0009-home-compatibility-admission.md)） |
| `<home>/run/host.lock/`（含 `owner.json` 与 `.dsh-writer-sentinel`） | `home-lease`                 | launcher 或 bundled CLI 在整个 Host writer 生命周期持有；哨兵使目录恒非空（布局 v2 单写者载体）并携带 supervisor 身份供 doctor 兜底判定                                                                                                               | 不是数据备份；只可按 owner 身份受控恢复                                                                                                                              |
