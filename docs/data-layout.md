...
| `<home>/run/compatibility.json`                     | 受支持写入者（M3 只读；M4 起 writer） | 任何受支持入口写入前只读校验；缺失允许，未知 schema/损坏/不支持 epoch 一律拒绝（exit/恢复页 fail-closed） | 无迁移：损坏即拒绝，不自动重写（[home-compatibility 协议](protocols/home-compatibility.md)、[ADR-0009](adr/0009-home-compatibility-admission.md)） |
