...
generation ledger（插件市场的前置能力）仍将由 `profile-manager` 拥有，但在其 schema、迁移和清理策略经 ADR 确定前不创建 generation 存储目录。transaction journal 与 generation ledger 的分离不改变 profile-manager 的唯一权威。
