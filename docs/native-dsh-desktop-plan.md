...
M1–M4 已拆成可交给 zcode 的逐任务文档，见 [执行路线与交接](superpowers/plans/2026-09-02-m1-m4-execution-roadmap.md)。这些计划以 M0 验收提交 `2f84e04` 为起点；M1、M2 已实施验收并合并 `main`；M3 已实施、通过 codex 复审与制品级验收并合并 `main`；M4 已实施并通过制品级演练验收（`codex/m4-release-compatibility`，见 [M4 验收记录](validation/m4-acceptance.md)）。Post-M4：DSH 0.1.2-rc.1 升级资格 GO（见 [rc.1 验收记录](validation/dsh-0.1.2-rc.1-acceptance.md)），M5 发行证据与插件引入已验收（见 [M5 验收记录](validation/m5-acceptance.md)）。
