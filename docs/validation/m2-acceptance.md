...
**codex 二审修复（2026-09-03，第三个修订）**：marker 写入失败不再静默继续 relaunch（预算未落盘 = 不消耗 relaunch，直接进恢复页，故障注入单测覆盖）；损坏的 marker 文件（解析失败）与未知 schema 一视同仁——预算视为已消耗且永不覆写/删除（marker store 移入 shell-core 成为共享实现并单测）；`fallbackFailure` 统一走 `toStartupFailure` 脱敏（loadSurface/commit/port 的普通异常不再绕过 token/home 打码，测试覆盖）；恢复页文案区分"home 权威数据未改"与"profile 候选已保留于 journal"；矩阵 `hostPid` 更名 `sessionPid` 并说明其语义；profile-recovery smoke 新增 cross-process 链（文件版 marker 串两个会话验证跨进程预算不被重置、健康后清除恢复）；projection-cache 损坏 journal 返回 unknown-layout 且字节不变（"不触碰未知数据"闭合）；credentials 摘要把 guidance 计入 1024 上限。
