...
4. **打包级演练**：`pnpm verify:plugin-intake` 用安装候选制品复演——记录校验 → 经候选自己的 `plugin --profile <新名> add` 流程把合成 fixture 只装入全新临时 profile → 对 staged 字节复验摘要 → 候选启动到 ui-ready → 默认 `desktop` profile 摘要前后逐字节一致。
