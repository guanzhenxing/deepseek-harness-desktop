...
格式勘察（`inspectHomeFormats`）只解析已知文件头/布局，不加载 DSH 或用户插件、不启动 provider：credentials 的 `version:` 头（`refs:` 与 `records:` 两种段落均为本 baseline 已知形态，前者 API-key 引用、后者桌面 Host 写入的连接授权）、settings 的存在性（provider build 即格式身份）、session JSONL 首行 `{type:'session',version:0}`（`.zstd` 按扩展名归类，本文 baseline 写入）、storage 单元信封（single：`{unit:{name,version}}`；per-record：`global.json` 为 `{version,record}`，域名取自目录名）、`session_projcache` 域固定 v4、profile manifest 的 `dsh.profile` 形状。**枚举与分类纪律（2026-09-06 修订，取代早期的 ≤32×32/≤64/≤32 采样上限）**：每一层目录用 `opendir` 增量枚举，超过统一上限（65,536，fail-closed）即把该目录计为 unknown 而不是只看前缀；枚举出的每个条目都做 lstat 外形检查（symlink/FIFO/异形一律计 unknown，绝不跟随）**并且**做头部分类——无法分类的正文在任何位置都拒绝，不存在"采样后放行"。`profiles/` 的豁免仅限真正的运行时启动根 `.dsh-desktop-run-*` **实目录**（同名的 symlink 照常拒绝；profile 名允许点号前缀，如 `.prod`，一律检查）；散落的普通文件（如 `.DS_Store`）不是 profile、运行时也不加载，跳过。`storages` 槽位固定为信封身份（域内增长不翻转槽位值），`projcache` 是独立槽位。
