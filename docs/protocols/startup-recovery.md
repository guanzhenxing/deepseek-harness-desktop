...
- **外层 lease 全程持有**：恢复窗口存活期间会话不释放 lease——手动重试与 Safe Mode 复用同一 lease；lease 仅在 quit 链（stop Host → 确认退出 → release）释放。release 自身失败时故意保留 lease 并提示 doctor。
- **正常链**：`lease → 恢复未完成 journal → cache 检查 → reconcile plan → 持久 journal → apply → Host ready → 窗口挂载 → committed`。committed 仅在 surface 挂载后写入。
- **失败链**：stop Host 并确认退出 → 分类 → 可归因且 SHA 匹配时回滚 → **最多一次自动正常重启**（relaunch-once）→ 恢复窗口。自动重启预算按事务 id 绑定 marker（`<userData>/recovery/<home 摘要>.json`）持久化，进程重启不重置；会话内亦只消耗一次。
